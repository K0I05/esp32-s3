...
static inline void LOGI_config_register(i2c_mlx90614_handle_t mlx90614_handle) {
    ...
    LOGI_config_register_binary(mlx90614_handle->config_reg.reg);

    if(mlx90614_handle->config_reg.bit.test_state == I2C_MLX90614_SENSOR_TEST_ENABLED) {
        ESP_LOGI(CONFIG_APP_TAG, "BIT-15 sensor test enabled");
    } else {
        ESP_LOGI(CONFIG_APP_TAG, "BIT-15 sensor test disabled");
    }

    if(mlx90614_handle->config_reg.bit.kt2_sign == I2C_MLX90614_KT2_SIGN_POSITIVE) {
        ESP_LOGI(CONFIG_APP_TAG, "BIT-14 KT2 sign positive");
    } else {
        ESP_LOGI(CONFIG_APP_TAG, "BIT-14 KT2 sign negative");
    }

    switch(mlx90614_handle->config_reg.bit.gain) {
    ...
    switch(mlx90614_handle->config_reg.bit.fir) {
    ...
    if(mlx90614_handle->config_reg.bit.k_sign == I2C_MLX90614_K_SIGN_POSITIVE) {
        ESP_LOGI(CONFIG_APP_TAG, "BIT-7 K sign positive");
    } else {
        ESP_LOGI(CONFIG_APP_TAG, "BIT-7 K sign negative");
    }

    switch(mlx90614_handle->config_reg.bit.t_sensors) {
        case I2C_MLX90614_GAIN_1:
    ...
    if(mlx90614_handle->config_reg.bit.test_repeat_state == I2C_MLX90614_SENSOR_TEST_REPEAT_ON) {
        ESP_LOGI(CONFIG_APP_TAG, "BIT-3 sensor repeat test on");
    } else {
        ESP_LOGI(CONFIG_APP_TAG, "BIT-3 sensor repeat test off");
    }

    switch(mlx90614_handle->config_reg.bit.iir) {
...
static inline void LOGI_pwmctrl_register(i2c_mlx90614_handle_t mlx90614_handle) {
    ...
    LOGI_config_register_binary(mlx90614_handle->pwmctrl_reg.reg);

    ESP_LOGI(CONFIG_APP_TAG, "BIT-15-9 PWM period %.3f ms", mlx90614_handle->pwmctrl_reg.pwm_period);

    ESP_LOGI(CONFIG_APP_TAG, "BIT-8-4 PWM repetition number is %u (0...62 step 2)", mlx90614_handle->pwmctrl_reg.bit.pwm_repetition);

    if(mlx90614_handle->pwmctrl_reg.bit.thermal_mode == I2C_MLX90614_THERMAL_MODE_PWM) {
        ESP_LOGI(CONFIG_APP_TAG, "BIT-3 PWM mode selected");
    } else {
        ESP_LOGI(CONFIG_APP_TAG, "BIT-3 Thermal relay mode selected");
    }

    if(mlx90614_handle->pwmctrl_reg.bit.sda_pin_mode == I2C_MLX90614_SDA_PIN_MODE_OPEN_DRAIN) {
    ...
    if(mlx90614_handle->pwmctrl_reg.bit.pwm_mode_state == I2C_MLX90614_PWM_MODE_STATE_DISABLED) {
        ESP_LOGI(CONFIG_APP_TAG, "BIT-1 PWM mode disabled");
    } else {
        ESP_LOGI(CONFIG_APP_TAG, "BIT-1 PWM mode enabled");
    }

    if(mlx90614_handle->pwmctrl_reg.bit.pwm_mode == I2C_MLX90614_PWM_MODE_EXTENDED) {
...
static void i2c_0_task( void *pvParameters ) {
    ...
    for ( ;; ) {
        ...
        ESP_LOGI(CONFIG_APP_TAG, "mlx90614 identification number (init), Hi(32-bit): %lu, Lo(32-bit): %lu", mlx90614_dev_hdl->ident_number_hi, mlx90614_dev_hdl->ident_number_lo);
        ESP_LOGI(CONFIG_APP_TAG, "mlx90614 object temperature range (init), maximum: %.2f C, minimum: %.2f C", mlx90614_dev_hdl->obj_max_temperature, mlx90614_dev_hdl->obj_min_temperature);
        ESP_LOGI(CONFIG_APP_TAG, "mlx90614 emissivity (0.1 to 1.0 - init):           %.2f", mlx90614_dev_hdl->emissivity);
        //
